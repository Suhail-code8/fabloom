    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 pb-4 lg:pb-0">
            ...
            <div className="lg:col-span-5 flex flex-col gap-4">
                <div className="relative aspect-[3/4] rounded-sm overflow-hidden bg-[rgba(255,255,255,0.05)] shadow-xl group">
                    <Image 
                    ...
                    />
                    {product.featured && (
                        <span className="absolute top-4 left-4 bg-[#D4A853] text-white text-[10px] font-extrabold uppercase tracking-widest px-3 py-1.5 rounded-full shadow-lg">Featured</span>
                    )}
                </div>
                    ...
                    <div className="flex gap-3 overflow-x-auto pb-2 custom-scrollbar">
                        {product.images.map((img, idx) => (
                            <button 
                                ...
                                className={`relative w-20 aspect-square rounded-sm overflow-hidden border-2 transition-all ${activeImage === idx ? 'border-[#D4A853] scale-95 shadow-md' : 'border-transparent opacity-60 hover:opacity-100'}`}
                            >
            ...
            <div className="lg:col-span-7 flex flex-col pt-2">
                <div className="mb-8">
                    ...
                    <p className="text-gray-300 leading-relaxed max-w-xl">{product.description}</p>
                </div>
                ...
                <div className="space-y-8 mb-10">
                    ...
                    {isReadymade && (
                        <div>
                            <div className="flex items-center justify-between mb-4">
                                <h3 className="text-xs font-extrabold uppercase tracking-widest text-white">Select Size</h3>
                                ...
                                {Object.entries((product as IReadymadeProduct).sizeStock || {}).map(([size, stock]) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`min-w-[56px] h-12 flex items-center justify-center rounded-sm border-2 font-bold text-sm transition-all ${
                                                selectedSize === size 
                                                    ? 'border-[#0f1035] bg-[#0f1035] text-white shadow-lg scale-95' 
                                                    : outOfStock 
                                                        ? 'border-gray-100 text-gray-300 cursor-not-allowed bg-[rgba(255,255,255,0.02)]' 
                    ...
                    {isFabric && (
                        <div className="space-y-8">
                            <div>
                                <h3 className="text-xs font-extrabold uppercase tracking-widest text-white mb-4">Quantity (Meters)</h3>
                                <div className="flex items-center gap-4">
                                    <div className="flex items-center bg-[rgba(255,255,255,0.05)] rounded-sm p-1 shadow-inner">
                                        ...
                                        <span className="w-16 text-center font-extrabold text-white">{meters}m</span>
                            ...
                            {(product as IFabricProduct).stitchingAvailable && (
                                <div className="bg-amber-50/50 rounded-sm p-6 border border-amber-100 shadow-sm">
                                    <div className="flex items-center justify-between mb-4">
                                        <div>
                    ...
                    {!isFabric && (
                        <div>
                            <h3 className="text-xs font-extrabold uppercase tracking-widest text-white mb-4">Quantity</h3>
                            <div className="flex items-center bg-[rgba(255,255,255,0.05)] rounded-sm p-1 shadow-inner w-fit">
                                ...
                                <span className="w-12 text-center font-extrabold text-white">{quantity}</span>
                ...
                {/* ADD TO CART ACTION */}
                <div className="flex flex-col gap-4 mt-auto pt-4 border-t border-gray-100 bg-transparent store-action-bar px-4 rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.06)] lg:static lg:px-0 lg:pb-0 lg:rounded-none lg:shadow-none lg:pt-8 lg:bg-transparent">
                    <button 
                        onClick={handleAddToCart}
                        disabled={isAdding}
                        className="ecom-btn w-full h-16 text-lg shadow-xl active:scale-[0.98] transition-all flex items-center justify-center gap-3 disabled:opacity-70"
                    >
                        {isAdding ? (
                    ...
                    <div className="flex items-center justify-center gap-6">
                        <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-[10px] font-bold text-gray-300 uppercase tracking-widest">Secure Checkout</span>
                        </div>
                        <div className="flex items-center gap-1.5">
                            <svg className="w-4 h-4 text-green-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2.5}><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
                            <span className="text-[10px] font-bold text-gray-300 uppercase tracking-widest">Free Shipping Over ₹999</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="mt-12 grid grid-cols-2 gap-4">
                    <div className="p-5 rounded-sm bg-[rgba(255,255,255,0.02)] border border-gray-100">
                    ...
                    <div className="p-5 rounded-sm bg-[rgba(255,255,255,0.02)] border border-gray-100">
                        ...
                        <p className="text-xs font-bold text-white">{isFabric && wantsStitching ? '7-12 Business Days' : '3-5 Business Days'}</p>
                    </div>
                </div>
            </div>
